...
<?php if ($ac == '') : ?>
	...
	$mapel = mysqli_fetch_array(mysqli_query($koneksi, "SELECT * FROM ujian WHERE id_ujian='$id_ujian'"));
				...
				<div class='box-body'>
					<div id="tablenilai" class='table-responsive'>
						<table id="tablenilaix" class='table table-bordered table-striped'>
							...
							<tbody>
								<?php $siswaQ = mysqli_query($koneksi, "SELECT * FROM siswa a join nilai b on a.id_siswa=b.id_siswa where b.id_ujian='$id_ujian' and b.id_mapel='$id_mapel' " . $sqllevel . $sqlkelas); ?>
								<?php while ($siswa = mysqli_fetch_array($siswaQ)) : ?>
									...
									$nilaiQ = mysqli_query($koneksi, "SELECT * FROM nilai WHERE id_ujian='$id_ujian' AND id_siswa='$siswa[id_siswa]'");
						...
						<iframe name='frameresult' src='report.php?m=<?= $id_ujian ?>&i=<?= $kode_ujian ?>&k=<?= $id_kelas ?>' style='border:none;width:1px;height:1px;'></iframe>
					</div>
				</div><!-- /.box-body -->
			</div><!-- /.box -->
		</div>
